...
while i_episode <= 9:
    ...
    while done == False:
        ...
        reward = sum(rewards) / len(
            rewards
        )  # average of the agent rewards since there are multiple
        rewards1 += rewards[0]
        rewards2 += rewards[1]
        rewards3 += rewards[2]
        rewards4 += rewards[3]
        ...
        if done:
            ...
            average_reward = average_reward + (episode_reward - average_reward) / (
                i_episode + 1
            )
            if i_episode % 1 == 0:
                print(
                    "Episode {}: average: {:.3f} last 100: {:.3f} current reward: {:.3f}".format(
                        i_episode, average_reward, last_100_average, episode_reward
                    )
                )
