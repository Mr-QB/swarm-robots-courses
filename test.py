...
def localizeND2file(
    ...
    with open(outfile, "w") as o:
        o.write(
            "particle_idx\tframe_idx\ty_coord_um\tx_coord_um\talpha\tSig2\toffset\tr\tresult_ok\tim_part_var\n"
        )
        particle_idx = 0
        for frame_idx in range(N_frames):
            ...
            sys.stdout.write(
                "Finished localizing %d/%d frames...\r" % (frame_idx + 1, N_frames)
            )
            sys.stdout.flush()
    ...
    if detections.sum() == 0:
        ...
        for i in range(n_detect):
            ...
            if detection_parameters[i, 3] > 0.0 and not test_bord:
                y_coord, x_coord, alpha, Sig2, offset, r, result_ok, naive_var = (
                    fitGaussian(
                        image_2d,
...
def fitGaussian(image, Pi, Pj, psf_std, window_size=9, ITER_MAX=50):
    half_window = int(np.floor(window_size / 2))
    im_part = image[
        Pi - half_window + 1 : Pi + half_window + 2,
    ...
    while test:
        r, i, j, dr, di, dj, alpha, sig2, offset = fitGaussianIter(
            r, i, j, im_part, sig2, dr, di, dj
        )
        cpt += 1
        ...
        if cpt > ITER_MAX:
            test = False
        result_ok = not (
            (i < localization_flags[0])
            or (i > localization_flags[1])
            or (j < localization_flags[2])
            or (j > localization_flags[3])
            or (r < localization_flags[4])
            or (r > localization_flags[5])
        )
...
def track(
    ...
    active_trajectories = [
        Trajectory(positions=[loc[2:4]], frames=[0]) for loc in locs[locs[:, 1] == 0]
    ]
    all_finished_trajectories = []
    for frame_idx in range(1, N_frames):
        active_trajectories, finished_trajectories, adjacency_graph = reconnectFrame(
            active_trajectories,
            locs,
            frame_idx,
            var_free,
        ...
        sys.stdout.write(
            "Tracked through %d/%d frames...\r" % (frame_idx + 1, N_frames)
        )
        sys.stdout.flush()
...
def saveTrajectoriesAsMat(trajectories, mat_file_name, frame_interval):
    result_list = []
...
def reconnectFrame(
    ...
    for traj_idx in range(N_traj):
        ...
        adjacency_graph[
            traj_idx,
            np.where(
                ((locs_in_frame[:, 2:4] - traj.positions[-1]) ** 2).sum(axis=1)
    ...
    for loc_idx in np.where(criterion)[0]:
        ...
        if log_likelihood >= pfa and locs_in_frame[loc_idx, 4] > minInt:
            new_trajectories.append(
                Trajectory(positions=[locs_in_frame[loc_idx, 2:4]], frames=[frame_idx])
            )

    # Break the remaining adjacency graph into smaller connected
    # components that are easier to solve
    subgraphs, traj_subgraph_labels, loc_subgraph_labels = connectedComponents(
        adjacency_graph
    )
    ...
    for subgraph_idx in range(len(subgraphs)):
        ...
        if subgraphs[subgraph_idx].shape == (1, 1):
            ...
            active_trajectories[traj_idx].positions.append(locs_in_frame[loc_idx, 2:4])
    ...
    finished_trajs = [
        traj for traj in active_trajectories if traj.N_blinks > max_blinks
    ]
    active_trajectories = [
        traj for traj in active_trajectories if traj.N_blinks <= max_blinks
    ] + new_trajectories
...
def initializeTrajectories(locs):
    ...
    for loc in first_frame_locs:
        result.append(Trajectory(positions=[loc[2:4]], frames=[0]))
    return result
...
def logLikelihoodReconnection(
    ...
    if trajectory.N_blinks != 0:
        sigma_blink = -T_off / 3
        P_blink = (2 / (sigma_blink * np.sqrt(2 * np.pi))) * np.exp(
            -(trajectory.N_blinks**2) / (2 * sigma_blink**2)
        )
    ...
    if particle_intensity < intensity_mean:
        ...
        P_int_univ = 0.0
    P_int_gaussian = (1 / (intensity_var * np.sqrt(2 * np.pi))) * np.exp(
        -((particle_intensity - intensity_mean) ** 2) / (2 * intensity_var)
    )

    L_intensity = np.log(
        intensity_law_weight * P_int_gaussian + (1 - intensity_law_weight) * P_int_univ
    )
    ...
    squared_displacement = (
        locs[particle_idx, 2] - trajectory.positions[-1][0]
    ) ** 2 + (locs[particle_idx, 3] - trajectory.positions[-1][1]) ** 2
    P_displacement_bound = (1 / (var_bound * np.sqrt(2 * np.pi))) * np.exp(
        -squared_displacement / (2 * var_bound)
    )
    P_displacement_free = (1 / (var_free * np.sqrt(2 * np.pi))) * np.exp(
        -squared_displacement / (2 * var_free)
    )
    L_displacement = np.log(
        diff_law_weight * P_displacement_bound
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "nd2_file", type=str, help="ND2 file or directory with ND2 files"
    )
    parser.add_argument(
        "-o",
        "--out_mat_file",
    ...
    parser.add_argument(
        "-p", "--pixel_size_um", type=float, help="um per pixel", default=0.16
    )
    parser.add_argument(
        "-f",
        "--frame_interval",
    ...
    parser.add_argument(
        "-n", "--NA", type=float, help="numerical aperture of microscope", default=1.49
    )
    parser.add_argument(
        "-s",
        "--searchExpFac",
    ...
    if args.track_only:
        ...
        if os.path.isdir(args.nd2_file):
            path_list = [
                "%s/%s" % (args.nd2_file, i)
                for i in os.listdir(args.nd2_file)
                if ".nd2" in i
            ]
